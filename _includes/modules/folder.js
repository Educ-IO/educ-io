Folder = function(folder, target, ಠ_ಠ) {
  ...
  /* <!-- Internal Variables --> */
	var _db = new loki("folders.db"), _results, _table;
  /* <!-- Internal Variables --> */

  /* <!-- Internal Functions --> */
	var mapItems = (v) => ({
		id : v.id, type : v.mimeType, name : v.name, parents : v.parents, icon : v.iconLink, thumbnail : v.thumbnailLink, 
		url : v.webViewLink, star : v.starred, folder : ಠ_ಠ.google.folders.is(v.mimeType), download : !!v.webContentLink, paths : v.paths
	});
	
	var _showData = function(id, name, values) {
		
		var headers = ["Star", "Name", "ID"].map((v, i) => ({
      name: v,
      ...
      hide_initially: false,
		...
		var _data = _db.addCollection(name, {indices: ["mimeType", "starred", "id", "name"]});
		_data.insert(values);
		
		_table = ಠ_ಠ.Datatable(ಠ_ಠ,  {id : folder.id, name: folder.name, data: _data, headers: headers}, {}, target.empty());
  ...
  var _showFolder = function(value) {
	...
	var _showResults = function(name, items) {
		...
		var _files = _.filter(items, (item) => item.mimeType !== "application/vnd.google-apps.folder");
	...
	var _convertFile = function(file, sourceMimeType, targetMimeType, prefixAfterConversion) {
		...
		return new Promise((resolve, reject) => {
			...
			var metadata = {mimeType : targetMimeType, name : file.name.substr(0, file.name.lastIndexOf(".")), parents: file.parents};
			...
			ಠ_ಠ.google.download(file.id).then(binary => {
				...
				ಠ_ಠ.google.upload(metadata, binary, sourceMimeType).then(uploadedFile => {
					...
					prefixAfterConversion ? 
						ಠ_ಠ.google.update(file.id, {name : prefixAfterConversion + file.name})
							.then(updated => resolve({new : uploadedFile, old: updated}))
		...
		return new Promise((resolve, reject) => {
			...
			var _updateSheet = function(id, last) {
				...
				ಠ_ಠ.google.sheets.append(id, "A" + last + ":F" + _total, values)
					.then((range) => {resolve({id: id, last: _total});})
					.catch(e => {reject(e);});
			...
			if (id) {
				...
				ಠ_ಠ.google.sheets.create("Folders - Conversion Results " + (folder && folder.name ? " for (" + folder.name + ") " : "") + "[" + new Date().toUTCString() + "]").then(sheet => {
					...
					var id = sheet.spreadsheetId, values = [];
	...
	var _convertItems = function() {
		...
		ಠ_ಠ.Display.modal("convert", {
			...
			if (_sourceMimeType && _targetMimeType) {
				...
				var _process_Batch = function(batch, batches, batch_index, length, id, last) {
					...
					var _process_Result = function(file, files, file_index, id, last, complete) {
						...
						if (file) {
							...
							ಠ_ಠ.Display.busy({target : _container.find(".file-name").parent(), class : "loader-small"});
							...
							}).catch(e => {_container.find(".file-name").addClass("action-failed text-danger font-weight-bold"); __result.__class = "action-failed text-danger font-weight-bold"; ಠ_ಠ.Flags.error("File " + file_index + " Conversion Error", e ? e : "No Inner Error"); _failures.push(file); ಠ_ಠ.Display.busy({clear: true}); _process_Result(files.shift(), files, file_index + 1, id, last, complete);});

						} else {

							ಠ_ಠ.Flags.log("FILE CONVERSION COMPLETE: " + _successes.length + " successfully converted, " + _failures.length + " failed to convert.");

							_saveConversionResults(_successes, _failures, id, last).then((v) => {complete(v);}).catch((e) => {complete();});
				...
				var batches = _.chain(_results).groupBy((v, i) => Math.floor(i / 4)).toArray().value();
