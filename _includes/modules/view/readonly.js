ReadOnly = (ಠ_ಠ, sheet, parameters) => {
  ...
  var _display = (data, target) => {
		...
		if (parameters.c && headers.length === 0) headers = Array.apply(null, {length: parameters.c});
    ...
    headers = _.map(headers, (v, i) => {
			...
			var n = v ? v : `-${i}-`, h = _.find(parameters.h, h => (_.isString(h.n) && h.n == n) || (_.isNumber(h.p) && h.p === i));
    ...
    var length = 0, values = _.map(data, v => {
    	length = Math.max(length, v.length);
			/* <!-- NOTE: Data could be a jagged array, so bulk out arrays if required --> */
			if (parameters.c && v.length < parameters.c) v = v.concat(Array.apply(null, {length: parameters.c - v.length}));
  ...
  var _fetch = (sheet, target) => {
    ...
    return ಠ_ಠ.Google.sheets.values(sheet.spreadsheetId, `${parameters.n}!A:ZZ`)
      .then(data => {
				...
				return _.extend(sheet, {data: data});
			})
			.then(sheet => {
				_busy();
				return sheet;
