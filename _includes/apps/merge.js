App = function() {
  ...
  var _pick = {
    ...
    merge: () => ಠ_ಠ.Main.pick("Select a Merge to Open",
    ...
    doc: () => ಠ_ಠ.Main.pick("Select a Document to Open",
    ...
    form: () => ಠ_ಠ.Main.pick("Select a Form to Open",
        ...
        route: (handled, command) => {
          ...
          if ((/OPEN/i).test(command)) {
            ...
            .then(file => ((/DATA/i).test(command[1]) ?
                  ಠ_ಠ.Records(ಠ_ಠ, file, $(`#${ID}_data_panel`), command[3])
                  .then(records => {
                    _records = records;
