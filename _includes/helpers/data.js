Data = (options, factory) => {
  ...
  /* <!-- @options.inputs: JQuery Filter to select input fields [Optional]  --> */
  ...
  var _dehydrate = form => {
    ...
    return _.reduce(_all, (values, input) => {
      ...
      var _$ = $(input),
        ...
        _multiple = _inputs.length > 0,
        _field = _.reduce(_multiple ? _inputs : [_$.is(":input") ? input : _$.find("input, textarea")[0]], (field, input) => {
          var _$ = $(input);
  ...
  var _rehydrate = (form, data) => {
    ...
    _.each(form.find(options.inputs), input => { /* <!-- Iterate through all the fields in the form --> */
      var _$ = $(input),
